  ...
  operations.map((op) => {
    if (op === "%") {
      ...
      if (operations.indexOf(op) === 1) {
        nums[mapIndex - 1] = nums[mapIndex - 1] / 100;
        nums.splice(mapIndex, 1); // taking out num so only calculated result remains
        operationsClone.splice(operationsClone.indexOf(op), 1); // removing operation
      } else {
        switch (operations[operations.indexOf(op) - 1]) {
          case "x":
            nums[mapIndex - 2] =
              (nums[mapIndex - 1] / 100) * nums[mapIndex - 2];
          ...
          case "/":
            nums[mapIndex - 2] =
              nums[mapIndex - 2] / (nums[mapIndex - 1] / 100);
            nums.splice(mapIndex - 1, 2); // removes the first number the operation was performed on leaving only the calcualted result
            // operationsClone.splice(operationsClone.indexOf(op), 1);
            operations.splice(operations[operations.indexOf(op) - 1]);
            break;
